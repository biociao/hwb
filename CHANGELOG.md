...
1. **代码在哪**：分支 `tmp-reorder2`，今晚新增 **100+ 个提交（本地，未 push）**；
   `main` 落后于本分支，**要不要合并/push 由你决定**。工作树是干净的、全套测试 623 例通过（1 skip）。
...
3. **建议先看的东西**：本文件顶部的「已知残留」段（每条都带实测数字、改法与「怎么证明改好了」）——
...
多轮针对「我自己刚改的代码」的自审，共 **130+ 个提交、623 个用例**
...
- **实时轮询每 3s 重写每个实例的每一行会话**（**已修一半**）：`applyLiveStatus` 走的是 `SELECT *` +
  全量 upsert，代价正比于该实例的**总会话数**而不是「变了多少」。实测 40k/10 实例：**891ms/轮**
  （约占每轮的 30%，期间事件循环被同步阻塞）；单个 home 有 200k 会话时**单次 3,494ms**。
  已修的部分：内容与库里**逐字节相同**时跳过整表写（空闲的 dsh 不再每 3s 白跑——那正是这条调查里
  最大的一块）；任何一处不同仍然走原来的整表替换路径，所以语义不变。
  仍留着：**部分变化**时依旧是全量重写（该实例成百上千个空闲会话陪着一起写）。
  改法：只写 `liveIds` 里真正变了的行（需要一个新的「按行 UPDATE」写入路径）。
