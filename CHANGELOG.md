...
对 v0.1.1 之后的全库做了一轮系统回顾：**19 轮独立对抗式审查**（控制平面 ×2 / lib+api /
...
多轮针对「我自己刚改的代码」的自审，共 **120+ 个提交、616 个用例**
...
- **实时轮询每 3s 重写每个实例的每一行会话**：`applyLiveStatus` 走的是 `SELECT *` + 全量 upsert，
  代价正比于该实例的**总会话数**而不是「变了多少」。实测 40k/10 实例：**891ms/轮**（约占每轮的 30%，
  期间事件循环被同步阻塞）；单个 home 有 200k 会话时**单次 3,494ms**。
  改法：只写 `liveIds` 里那几行（集合已经有了），或对 status/lastActivity 未变的行跳过。
  证明：400k 夹具上断言 `applyLiveStatus` 随 `live.length` 而不是 `sessionCount` 增长（<100ms）。
- **上传把整个 body 缓存在内存里，且没有并发上限**：实测 256 MiB（`UPLOAD_BYTES` 上限）单次峰值
  **1.12 GiB RSS（4.38×）**；4 个 32 MiB 并发 **557 MB**；4 个满额并发 ⇒ ~4.4 GiB ⇒ 容器 OOM
  （hwb 没有设置堆上限）。改法：给上传加并发闸门（同一 ws 同时只允许一个，超了 503），
  并/或让路由的 `write(chunk)` 直接喂给 uploader（它本身是有背压的流）。
  证明：4 个 256 MiB 并发上传，峰值 RSS < 1.5 GiB。
- **`/api/usage` 的实测成本高于项目自己文档里的数字**：40k 夹具上 8 个聚合约 **148ms**
