...
多轮针对「我自己刚改的代码」的自审，共 **130+ 个提交、624 个用例**
...
- **修复**：sessions 的替换只作用于**文件快照该管的那部分**（`WHERE homeId = ? AND liveOnly = 0`）。
  纯实时行的生命周期完全交给 `applyLiveStatus`：不在实时列表里就删、被文件索引收录后由已有的
  `ON CONFLICT` 分支把 `liveOnly` 归零。两边的语义本来就一致，只是替换范围原先划大了。
- **回归测试**：`tests/live-empty-clear.test.js` —— 文件索引重跑后纯实时行仍在；紧接着
  「实时列表变空 ⇒ 它必须被清掉」这条原有语义也一并断言（防止改坏）。修复前失败。
