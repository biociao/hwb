...
对 v0.1.1 之后的全库做了一轮系统回顾：**8 轮独立对抗式审查**（控制平面 / lib+api / web+dshhome /
文档一致性 / 服务生命周期 / 预览与静态缓存与远端索引 / 前端与 SSE / 渲染壳与日志脚本）
\+ 1 轮纯函数对抗 fuzz + 2 轮敌意环境测试 + 多轮针对「我自己刚改的代码」的自审，
共 60+ 个提交、516 个用例。**最高影响**的几条（都有实测复现与回归测试）。
后两类是**第 5–8 轮**审查才发现的 —— 前四轮已经「审完」了同一批文件，这本身就是一条教训：
一次审查通过不代表没有缺陷，只是那一轮的选题没碰到它。
