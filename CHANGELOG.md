...
对 v0.1.1 之后的全库做了一轮系统回顾：**12 轮独立对抗式审查**（控制平面 / lib+api / web+dshhome /
文档一致性 / 服务生命周期 / 预览与静态缓存与远端索引 / 前端与 SSE / 渲染壳与日志脚本 /
跨模块数据流 / 存储层 / 近两轮改动的回归审查）+ 1 轮纯函数对抗 fuzz + 2 轮敌意环境测试
\+ 多轮针对「我自己刚改的代码」的自审，共 80+ 个提交、553 个用例。
**最高影响**的几条（都有实测复现与回归测试）。
