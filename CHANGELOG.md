...
多轮针对「本次刚改的代码」的自审，共 **145 个提交、638 个用例**
...
### Fixed
#### 「按 Model」不再假装是事实：标注「（档位推定）」与「未识别」（2026-09-13）
- **现象（用户实测截图）**：图例只有 `deepseek-v4-flash-vision-exp` 与 `unknown` 两项，
  而同期实际用过 7 个模型；用户问「模型识别上怎么不能识别其他模型了？」。
- **根因**：「按 Model / 按 provider」维度的取值来源是 `model_tiers` —— 该实例**当前**
  `model-tier.json` 的 default 档，而该表每次索引整表重建。于是它既不是该会话真实用过的模型，
  又会被一次「换默认模型」连带改写**历史**；没有 model-tier.json 的实例（bee / CMS / c4g.tun /
  dgx21.tun，合计 631M ≈ 17% 的 token）则一律 `unknown`，在图例里长得就像一个模型名。
- **为什么不去读会话日志取真实模型（已实现并实测有效，随后整体回退）**：
  真实模型确实只在 `sessions/<项目>/<会话>/session.jsonl.zstd` 里。2026-09-13 实现了
  `src/dshhome/session-models.js`（多帧 zstd 切帧 + 按 offset 增量解压 + 帧间让出事件循环 +
  挂在索引轮次之后），实测 Model 维度 2 项 → 7 项、最大项从 `deepseek-v4-flash-vision-exp 3006M`
  纠正为 `deepseek-v4.1-flash-expires-on-0910 1886M`（窗口内 394 个本地会话有 374 个拿到真实模型）。
  **但它与项目的硬性规则「永不碰 `*.zstd`」直接冲突**（README「Reader 只读取以下 4 个文件」、
  `docs/topology.md`、`DSH_Workbench_Fusion_Architecture.md` 共 8 处：工作台活在投影缓存第一层、
  绝不下探日志），因此按用户决定**整体回退**（实现留在提交 `fd4cd42` 的历史里，可用 reflog 找回）。
  回退而不是放宽规则的理由，恰好被这次实现自己验证了：日志格式是 dsh 的内部细节 ——
  多帧拼接的 zstd 让 `zlib.zstdDecompressSync` **只解第一帧就静默返回**（实测 21MB 文件只吐
  196 字节且不报错），必须手写帧头扫描才读得动，正是「不下探日志格式」要防的那种耦合。
- **保留的改动（不碰 `.zstd`，只把话说准）**：
  · `usageTrendGrouped` 的 Model 维度给档位值加 **`（档位推定）`** 后缀 —— 一眼能看出这不是事实，
    而是拿当前配置倒推的；读不到档位配置的实例仍是 `unknown`；
  · 卡片图例与悬停里 `unknown` 显示为「**未识别（该实例未配置模型档位）**」。
- **回归测试**：`tests/store.test.js` 的 Model 维度用例改为断言「（档位推定）」后缀**必须存在**
  —— 这条断言防的是「以后有人把后缀去掉、让推测值重新冒充事实」。
- **残留**：Model 维度在语义上仍是**推测**，只是不再伪装。要拿到真实模型，得先决定是否放宽
  「永不碰 `*.zstd`」这条硬性规则（`fd4cd42` 是一份可用的实现）。
：空桶断线把 14 天的曲线切成十来段（2026-09-13）
- **现象（用户实测截图）**：14 天窗口「用量趋势 · 按 Model」的折线被切成十来段短线段，像图缺了一块。
- **根因**：`usageTrendHtml` 先 `filter(b => b.total > 0)` 丢掉空桶，再在相邻非空桶间隔超过
  1.5 个桶宽时**主动断线**（当时的理由是「跨空白连一条平滑曲线等于替用户编一段逐步衰减」）。
  而真实数据里空桶占 **19/56（34%）**、断点 11 处 —— 因为 token 是**按会话累计**记在
  `lastActivity` 那一个桶里的：一个跨两天的长会话，全部 token 落在它结束的那一格，
  中间全是 0，于是几乎每两个点之间都隔着空桶。
- **修复**：空桶**照画并按 0 连线**（一个分组一条连续曲线）。代价由**图注**接手、不能悄悄消失：
  有空桶时额外输出一行「其中 N 格无数据，按 0 计入并连线 —— 0 表示该时段没有会话结束，
  不等于该时段没有用量」。0 值仍然不画散点（按维度拆分时基线上一排重叠圆点，
  同一位置悬停只能命中 DOM 里最后一个）。
- **回归测试**：`tests/web-render-safety.test.js` 的两条趋势图用例按新口径重写
  —— 跨 21 个空桶必须仍是 **1 条 path**、空桶坐标落在基线 y=100、图注必须写明 0 的含义；
  另一条改为断言标签与散点共用同一个「时间 → x%」映射（不再依赖「标签只落在非空桶上」这个旧前提）。
...
- `hwb doctor` 原先把版本判断内联成 `major < 22 || (major === 22 && minor < 5)`，与新模块重复；
  改为复用同一判断，避免两处漂移。
- **端到端复验**：本机另有一个 Node **20.11.0**（低于门槛、且没有 `node:sqlite`），
...
- 先用真实 HTTP 探了一遍（相对越界 / 绝对路径 / 符号链接文件 / 符号链接目录 / 内嵌 `..` /
  URL 编码的 `..%2F`）：download 与 preview 全部 400 且不回内容，正常文件 200 —— 然后把这份探针
  固化成测试（含 `sec-fetch-site: cross-site` → 403、未知 workspaceId 不泄内容两条对照）。

### 已知残留（规模审查实测：有数字、有改法，尚未修）
这几条是**测出来的**、有明确改法，但改动面大到无法在发布窗口内安全验证，
所以先记下数字与改法，留待后续（每条都注明了「怎么证明改好了」）：
- **CI 里没有「真浏览器渲染」这一步**（补文档时发现的覆盖缺口，不是新缺陷）：
  CI 跑的是 `node scripts/smoke-e2e.mjs --port 4394` —— **17 条**断言，覆盖整条 API 链路；
  另外 **2 条**（真浏览器加载工作台、界面上能看到降级警示 chip）只在 `--with-chrome` 时跑。
  也就是说「CI 全绿」**推不出**「页面能正常渲染」，前端那几处修复（坐标轴错位、空状态少按钮）
  仍然只能手工用 `scripts/render-check.mjs` 验。
  改法：CI 里装一次 Chrome（或复用 `scripts/render-check.mjs` 的探测逻辑，找到才跑、找不到就**显式跳过并说明**）——
  关键不是「必须跑」，而是**不能静默等于通过**。
  证明：CI 日志里能看到这 2 条要么通过、要么写明「未找到 Chrome，已跳过」，不会只是消失。
...
- 一条命令跑完整条链路并逐步断言：起**隔离**实例（显式 `--db/--log`）→ 造一个「更早用过」的假 dsh home
  ...
  `SIGTERM` 优雅退出。实测 **19/19** 通过（**带 `--with-chrome`**；该开关会额外跑两条真浏览器断言。
  不带开关是 **17/17** —— 覆盖同一条链路的 API 层，但不去加载页面。CI 跑的是 17 条那一步，
  所以「CI 绿」不能推出「页面也渲染正常」，这两条断言目前仍需手工跑；见下面的已知残留。）
...
- 用它端到端验证了下面几条前端修复（隔离实例 + 假 home，真实数据、真实 SSE）：
...
- **根因**：`package.json` 声明了 `bin: { hwb: src/cli.js }`，但 git **从来没有**把该文件记为 `100755`
  ...
  （诚实标注：无法从 git 断定是谁清掉的，但那段时间多次重写 `src/cli.js`、写文件的工具落 0644 —— 这很可能就是原因。）
...
- **现象**（真机 fuzz 发现）：`curl -d 'null' -H 'content-type: application/json' /api/homes`
...
- **发布前又用最新代码复验了一遍**（此时已过了 20+ 个后续提交，包括 liveOnly 的保留策略与
...
- **来源**：用真实数据对照时发现的 —— 用户那台机器的 dsh 实时列表有 **500** 条会话，
...
- **来源**：复查上一条修复时发现的 —— 记录端口是**线索**，但 `stop` 把它与配置端口
...
- **一行缺字段拖垮该 home 的整批写入**：node:sqlite 拒绝绑定 `undefined`，抛
  ...
  **注意**：我第一版是「遍历键把 undefined 换成 null」，那是错的 —— 字段**整个缺失**时键根本不出现，
  所以那种做法兜不住（测试立刻证明了这一点）。测试：三行里有一行缺字段，三行都必须写入。
...
- **现象**（复查 `quota.js` 时发现，实测复现）：`logger(scope)` 返回的是**对象**
...
- **修复**：两处改成 `log.warn(...)`；`logBalanceFailure` 改走结构化日志（脱敏 + 进 UI），
  ...
  （把带 key 的错误消息喂进去，它确实原样进了日志）；`quota.js` 另外记一条带
...
- **修复**：① 把「点击 → 上报」抽成 `handleFileClick`，**`window` 与 `document` 各挂一次**
  ...
  ③ 漏拦兜底：万一某次点击仍被交给 dsh 的原生打开（远端必然失败），**不在会话正文里插任何控件**
  （第一版曾在失败提示的容器里插按钮，结果正文中提到该路径的每个容器都被插了一个，
  污染了对话——已按用户反馈移除），只在 iframe 右下角浮一条会自行消失的小提示并提供一个
  「在 hwb 文件预览里打开」的动作。
