...
> **收尾说明（轮次用尽，非 07:00）**：这轮自动回顾在工作树的**干净状态**上收尾 ——
> 全套测试 637 例（636 通过 / 1 skip / 0 失败）、冒烟 17/17、`git status` 无改动、无遗留探针进程。
> 收尾时刻是 **06:49**，比原定 07:00 早约 11 分钟：触发器的轮次上限（30 轮）先于墙钟用尽。
> 也就是说这是一次「按预算停止」，不是「按时间停止」—— 想继续只要再说一句「继续回顾」即可，
> 下面 `### 已知残留` 里每一条都带实测数字、改法与验收标准，可以直接接着做。
...
4. **建议先看的东西**：搜 `### 已知残留`（每条都带实测数字、改法与「怎么证明改好了」）——
   ...
   另外两个可复用的工具：`scripts/smoke-e2e.mjs`（一条命令跑完整条链路：17/17，加 `--with-chrome` 为 19/19）与
