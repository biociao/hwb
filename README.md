...
- **dsh token 的暴露面**：`GET /api/homes` 不再回传实例级 `token`；连接端点的
  `endpoints[].token` 仍会回传 —— 端点编辑器需要它做预填，否则「保存」会把 token 抹掉
  （要收掉必须同步改编辑器的预填与「清除」语义，测试里对现状有显式断言）。打开实例必经的
  `POST /homes/{id}/open` 会返回**带 token 的 iframe 入口**（dsh 的入口 URL 就靠它认证）：
  token 迟早要交给浏览器，所以这一条的边界是「谁能访问这个端口」，不是「响应里有没有这个字段」。
